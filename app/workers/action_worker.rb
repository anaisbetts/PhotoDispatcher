...
class ActionWorker < Workling::Base

  def invoke(options = {})
    action = options[:action]
    unless action.can_invoke? options
      logger.warn "Can't invoke, #{action.action_name} failed" 
      return
    end

    action.invoke(options)

    Actionentry.create do |ae|
      ae.action = action.action_name
      ae.photo_id = options[:item].id
    end
  end
